...
class FacebookScriptNode(template.Node):
        def render(self, context):
            ...
            """ % (settings.FACEBOOK_API_KEY, request.build_absolute_uri(reverse('xd_receiver')), request.build_absolute_uri(reverse('facebook_connect_ajax')))


def facebook_connect_script(parser, token): 
    return FacebookScriptNode()
...
def facebook_connect_login_button(parser, token):
    return FacebookLoginNode()
